...
#                    print("=======inv_obj===if=>>>",rec.invoice_id )
            else:
...
#            res = self.env.ref('account.action_invoice_tree1') odoo13
            ...
            res = res.read()[0]
            res['domain'] = str([('id','in',rec.custom_account_invoice_ids.ids)])
        return res
...
#    @api.multi odoo13
    def show_task(self):
        for rec in self:
            ...
            res = res.read()[0]
            ticket_task_id = self.env['project.task'].search([('ticket_id', '=', rec.id)])
            task_ticket_list = list(dict.fromkeys(rec.custom_project_task_ids.ids + ticket_task_id.ids))
