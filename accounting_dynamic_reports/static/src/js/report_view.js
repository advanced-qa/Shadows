odoo.define('accounting_dynamic_reports.MyReportWidget', function (require) {
    ...
    var report_bs_view = AbstractAction.extend({
        events: {
            'dblclick .categ': 'line_click',
            'click .o_account_financial_reports_print': 'print_pdf',
            'click .o_account_financial_reports_excel': 'export_xlsx',
        },
        ...
        */
        find_child: function (el, line, line_type, self) {
            ...
            console.log("line_detail", line_detail)
        ...
        /*This function is to print report view in pdf*/
        print_pdf: function () {
            var self = this;
            var html_data = document.getElementById('report_data').outerHTML
            this._rpc({
                model: this.searchModelConfig.context.active_model,
                method: 'print_pdf',
                args: [this.searchModelConfig.context.active_id, html_data],
            }).then(function (result) {
                self.do_action(result);
            });
        },
        /*This function is to export the current view to xlsx*/
        export_xlsx: function () {
            $("#report_data").table2excel({
                filename: "Accounting Report.xls"
            });
        },
...
//                  folding the row
        ...
        init: function(parent, action) {
            ...
            this.category = {};
            this.report_name = action.params.report_name;
            //id of parent
            this.id = action.id;
            this.parent = action.params.parent;
            this.report_id = action.params.report_id;
            var lines = action.params.report_lines;
            this.form = action.params.form;
            for (var l in lines){
                lines[l]['fold'] = true;
            }
            this.category = lines;
            this.currency = action.params.currency;
            this.journal_items = action.params.journal_items;
            return this._super.apply(this, arguments);
        },
        renderElement: function () {
            this._super();
            var self = this;
            console.log("self.parent, self.id", self, self.id)
